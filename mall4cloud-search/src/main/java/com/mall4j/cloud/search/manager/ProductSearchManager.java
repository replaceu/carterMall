...
public class ProductSearchManager {
	...
	private EsPageVO<ProductSearchVO> buildSearchResult(EsPageDTO pageDTO, SearchResponse response) {
		...
		// 总页码
		int totalPages = (int) total % pageDTO.getPageSize() == 0 ? (int) total / pageDTO.getPageSize() : ((int) total / pageDTO.getPageSize() + 1);
	...
	private void agg(ProductSearchDTO param, SearchSourceBuilder searchSourceBuilder, Boolean isAgg) {
		...
		if (Objects.isNull(isAgg) || !isAgg) { return; }
	...
	private void keywordSearch(ProductSearchDTO param, BoolQueryBuilder boolQueryBuilder) {
		if (StrUtil.isBlank(param.getKeyword())) { return; }
	...
	private void sort(ProductSearchDTO param, SearchSourceBuilder searchSourceBuilder, BoolQueryBuilder boolQueryBuilder) {
		...
		if (Objects.isNull(param.getSort())) {
			...
			FunctionScoreQueryBuilder functionScoreQueryBuilder = QueryBuilders.functionScoreQuery(boolQueryBuilder, ArrayUtil.toArray(filterFunctionBuilders, FunctionScoreQueryBuilder.FilterFunctionBuilder.class)).scoreMode(FunctionScoreQuery.ScoreMode.SUM).boostMode(CombineFunction.SUM);
	...
	private void loadSpuStatus(ProductSearchDTO productSearchDTO) {
		if (Objects.isNull(productSearchDTO) || Objects.isNull(productSearchDTO.getDataType())) { return; }
